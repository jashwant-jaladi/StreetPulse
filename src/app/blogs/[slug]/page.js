...
const BlogDetails = async({params}) => {
    const posts = await fetch(`http://localhost:8080/blogs/${params.slug}`).then((res) => res.json())
    return(
        ...
        {posts.id==5 ? <img src={posts.image} className='w-full h-full object-center object-cover transition-transform ease-linear duration-300 hover:scale-110 rounded-lg '/> : <img src={posts.image} className='w-full h-full object-top object-cover transition-transform ease-linear duration-300 hover:scale-110 rounded-lg '/>}
        </div>
        </div>
